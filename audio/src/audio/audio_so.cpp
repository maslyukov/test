...
// This is kinda ugly, but for simplicity I've left these as globals just like
// in the sample, as there's not really any use case for this where we have
// multiple audio devices yet.
...
/**
 * int(*)(short *buffer, int num_samples)
 */
...
// and then render the next. Hopefully it's okay to spend time in this callback
// after having enqueued.
static void bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq, void *context) {
    ...
    memset(buffer[curBuffer] + renderedFrames * 2, 0,
            (framesPerBuffer - renderedFrames) * 4);
    SLresult result = (*bqPlayerBufferQueue)->Enqueue(bqPlayerBufferQueue,
            buffer[curBuffer], sizeInBytes);
...
extern "C" bool OpenSLWrap_Init(AndroidAudioCallback cb, int _FramesPerBuffer,
        int _SampleRate) {
    audioCallback = cb;
    ...
    // Render and enqueue a first buffer. (or should we just play the buffer
    // empty?)
    curBuffer = 0;
    audioCallback(buffer[curBuffer], framesPerBuffer);

    result = (*bqPlayerBufferQueue)->Enqueue(bqPlayerBufferQueue,
            buffer[curBuffer], sizeof(buffer[curBuffer]));
