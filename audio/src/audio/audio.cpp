...
bool AndroidAudio_Init(AndroidAudioCallback cb, std::string libraryDir,
        int optimalFramesPerBuffer, int optimalSampleRate) {
    if (state != 0) {
    ...
    state->shutdown_func = (OpenSLWrap_Shutdown_T) dlsym(state->so,
            "OpenSLWrap_Shutdown");
    state->frames_per_buffer =
            optimalFramesPerBuffer ? optimalFramesPerBuffer : 256;
    state->sample_rate = optimalSampleRate ? optimalSampleRate : 44100;

    ILOG("OpenSLWrap init_func: %p   shutdown_func: %p",
            (void * )state->init_func, (void * )state->shutdown_func);
