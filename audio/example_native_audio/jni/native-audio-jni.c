...
// engine interfaces
static SLObjectItf engineObject = NULL; // Object interface
static SLEngineItf engineEngine;        // Engine interface

// output mix interfaces
static SLObjectItf outputMixObject = NULL;  // Object interface
static SLEnvironmentalReverbItf outputMixEnvironmentalReverb = NULL;

// buffer queue player interfaces
static SLObjectItf bqPlayerObject = NULL;   // Object interface
static SLPlayItf bqPlayerPlay;              // Play interface
static SLAndroidSimpleBufferQueueItf bqPlayerBufferQueue;
static SLEffectSendItf bqPlayerEffectSend;  // Effect send interface
static SLMuteSoloItf bqPlayerMuteSolo;      // mute solo interface
...
void bqRecorderCallback(SLAndroidSimpleBufferQueueItf bq, void *context) {
	...
	// the next buffer to fill but instead, this is a one-time buffer so
	// we stop recording
	SLresult result;
...
void Java_com_example_nativeaudio_NativeAudio_createEngine(JNIEnv* env,
	...
	// either because the feature is not present, excessive CPU load, or the
	// required MODIFY_AUDIO_SETTINGS permission was not requested and granted
...
void Java_com_example_nativeaudio_NativeAudio_createBufferQueueAudioPlayer(
	...
	// create audio player
	const SLInterfaceID ids[3] = {          // Array of 3 Interfaces
			SL_IID_BUFFERQUEUE,             // Buffer queue interface (pointer)
			SL_IID_EFFECTSEND,              // Effect send interface (pointer)
			/*SL_IID_MUTESOLO,*/            //
			SL_IID_VOLUME                   // Volume interface (pointer)
	};
	const SLboolean req[3] = {              // Array of 3 flags
...
jboolean Java_com_example_nativeaudio_NativeAudio_createUriAudioPlayer(
	...
	SLDataLocator_URI loc_uri = {
	        SL_DATALOCATOR_URI,     // Locator type
	        (SLchar *) utf8         // URI (universal resource identificator)
	};
	SLDataFormat_MIME format_mime = {
	        SL_DATAFORMAT_MIME,             // Format type
	        NULL,                           // mimeType
	        SL_CONTAINERTYPE_UNSPECIFIED    // Container type
	};
	SLDataSource audioSrc = {
	        &loc_uri,                       // pointer to locator
	        &format_mime                    // pointer to format
	};
	// configure audio sink
	SLDataLocator_OutputMix loc_outmix = {
	        SL_DATALOCATOR_OUTPUTMIX,       // Locator type
	...
	// prepare/prefetch on Android, or possibly during Realize
	// on other platforms
	assert(SL_RESULT_SUCCESS == result);
	(void) result;
...
void Java_com_example_nativeaudio_NativeAudio_setPlayingUriAudioPlayer(
...
void Java_com_example_nativeaudio_NativeAudio_setLoopingUriAudioPlayer(
	...
	if (NULL != uriPlayerSeek) {
		...
		result = (*uriPlayerSeek)->SetLoop(uriPlayerSeek, (SLboolean) isLooping,
				0,
				SL_TIME_UNKNOWN);
		assert(SL_RESULT_SUCCESS == result);
		(void) result;
...
void Java_com_example_nativeaudio_NativeAudio_setChannelMuteUriAudioPlayer(
...
void Java_com_example_nativeaudio_NativeAudio_setChannelSoloUriAudioPlayer(
...
int Java_com_example_nativeaudio_NativeAudio_getNumChannelsUriAudioPlayer(
		JNIEnv* env, jclass clazz) {
	SLuint8 numChannels;
	SLresult result;
	SLMuteSoloItf muteSoloItf = getMuteSolo();
...
void Java_com_example_nativeaudio_NativeAudio_setVolumeUriAudioPlayer(
...
void Java_com_example_nativeaudio_NativeAudio_setMuteUriAudioPlayer(JNIEnv* env,
		jclass clazz, jboolean mute) {
	SLresult result;
	SLVolumeItf volumeItf = getVolume();
	if (NULL != volumeItf) {
...
void Java_com_example_nativeaudio_NativeAudio_setStereoPositionUriAudioPlayer(
...
jboolean Java_com_example_nativeaudio_NativeAudio_enableReverb(JNIEnv* env,
	...
	// and even if environmental reverb was present, it might no longer
	// be available
	if (SL_RESULT_SUCCESS != result) {
		return JNI_FALSE;
	}
...
jboolean Java_com_example_nativeaudio_NativeAudio_selectClip(JNIEnv* env,
	...
	case 4:     // CLIP_PLAYBACK
		// we recorded at 16 kHz, but are playing buffers at 8 Khz,
		// so do a primitive down-sample
	...
	if (nextSize > 0) {
		...
		// but for streaming playback we would typically enqueue at
		// least 2 buffers to start
		SLresult result;
		result = (*bqPlayerBufferQueue)->Enqueue(bqPlayerBufferQueue,
				nextBuffer, nextSize);
		if (SL_RESULT_SUCCESS != result) {
...
void Java_com_example_nativeaudio_NativeAudio_startRecording(JNIEnv* env,
	...
	// (for streaming recording, we would enqueue at least 2
	// empty buffers to start things off)
